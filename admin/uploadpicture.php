...
$account = mysqli_fetch_array($account1);
	...
	$query = $MySQLi_CON->query("SELECT  password, ppic FROM admin WHERE email='$_SESSION[userSessionAdmin]'");
	...
	if(isset($_POST['upload_pic']))
	{
		...
		$stmt_select = $MySQLi_CON->query("SELECT ppic FROM admin WHERE email='$_SESSION[userSessionAdmin]'");
	//	$stmt_select->execute(array('$_SESSION[userSessionAdmin]'=>$_GET['upload_pic']));
		$imgRow=$stmt_select->fetch_array();
		unlink("../assets/images/".$imgRow['ppic']); }

			$stmt = $MySQLi_CON->prepare("UPDATE  admin SET ppic='$ppic' WHERE email='$_SESSION[userSessionAdmin]'");
			...
			if($stmt->execute())
			{
				...
				header("refresh:5;account"); // redirects image view page after 5 seconds.
			}
			else
			{
				$errMSG = "<div class='alert bg-info'>
...
<form method="post" enctype="multipart/form-data" class="form-horizontal">
	   <div class="col-md-6" >
        ...
        <input class="input-group" type="file" name="p_pic" accept="image/*" /><br><br>
