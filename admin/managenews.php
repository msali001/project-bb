...
$t=mysqli_query($MySQLi_CON,"select * from bloodnews ORDER BY id DESC");
...
$query=mysqli_query($MySQLi_CON,"select * from bloodnews ORDER BY id DESC limit $start,$limit");
?>
<!DOCTYPE html>
<html>
<head>
    ...
    <div class="container">
        <form method="post" name="frm">
            <table class='table table-bordered table-responsive'>
                ...
                <?php
                $res = $MySQLi_CON->query("SELECT * FROM bloodnews ORDER BY id DESC");
                $count = $res->num_rows;
                if($count > 0) {
                        ...
                        <tr >
                            <td style="border-style: none;">
                                <span><input type="checkbox" name="chk[]" class="chk-box" value="<?= $ft['0'] ?>"/>
                                    <h2 class="blog-post-title"><?= $ft['1'] ?></h2></span>
                                <p class="blog-post-meta">Written by <a href="#" class="a-red"> <?= $ft['2'] ?> </a> on
                                    <span class="a-red"><?= $ft['8'] ?> </span></p>
