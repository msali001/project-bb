...
if(isset($_POST['btn-register']))
{
	...
	$group = $MySQLi_CON->real_escape_string(trim($_POST['group']));
	...
	if($count==0){
		...
		$query = "INSERT INTO donarregister(fname,lname,email,mobile,password, gender,day,month,year,county, b_id)
 					VALUES('$fname','$lname','$email','$mobile','$new_password','$gender','$day','$month','$year','$county', '$group')";
		...
		{
			$msg1 = "<div class='alert bg-info'>
						<button class='close' data-dismiss='alert'>&times;</button>".$MySQLi_CON->error." Error while registering ".$query." !!!
					</div>";
		}
	}
	else{
